...
function updateMessages() {        
    $(".messagehistory").load("http://better-planet.org/messenger/loadscripts/messenger_script.php", function( response, status, xhr ) {
        if ( status == "error" ) {
...
function updateMessageHeader() {        
    $(".messageHeader").load("http://better-planet.org/messenger/loadscripts/messenger_header.php", function( response, status, xhr ) {
        if ( status == "error" ) {
...
function updateMessageInput() {        
    $(".messenger").load("http://better-planet.org/messenger/loadscripts/messenger_input.php", function( response, status, xhr ) {
        if ( status == "error" ) {
            var msg = 'Loading message header resulted in an error: ';
             console.log(msg + xhr.status + " " + xhr.statusText );
         }
...
function loadUsers() {        
    $("#userList").load("http://better-planet.org/messenger/loadscripts/get_users_script.php", function( response, status, xhr ) {
        if ( status == "error" ) {
...
function loadConversations() {        
    $("#convoList").load("http://better-planet.org/messenger/loadscripts/get_conversations_script.php", function( response, status, xhr ) {
        if ( status == "error" ) {
            var msg = 'Loading users resulted in an error: ';
            console.log(msg + xhr.status + " " + xhr.statusText );
        }
...
$.ajax({
    type: 'post',
    url: 'http://better-planet.org/messenger/loadscripts/messageAjax.php',
    data: {
...
function searchUsers() {
 ...
 $.ajax({
  type: 'post',
  url: 'http://better-planet.org/messenger/loadscripts/searchUsersAjax.php',
  data: {
